...
// TODO: maybe split in 3 enums? distances, weights and temperatures?
...
fun main() {
    while (true) {
        ...
        // error 2: both are known/correct but incompatible (e.g. length <> weight)
        if (from.type != to.type) {
            println("Conversion from ${plural(from.normalizedName)} to ${plural(to.normalizedName)} is impossible")
            continue
        }

        // error 3: length and weight must not be negative
        if (fromValue < 0.0) {
        ...
        // TODO: not very nice, maybe put formula in enum somehow??
